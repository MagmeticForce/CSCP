...
exportToPDF = () => {
  ...
  doc.text("Your Personalized Schedule", 20, 20);
  
  const columns = ["#", "Course Name", "Course Code"];
  
  const rows = this.state.courses.map((course, index) => [
    index + 1, 
    course.name, 
    course.code,  
  ]);
  ...
  exportToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(this.state.courses.map(course => ({
      Name: course.name,
      Code: course.code,
      ...
      <DragDropContext onDragEnd={this.onDragEnd}>
        <div className="App">
          ...
          <div className="export-buttons">
            <button className="export-pdf" onClick={this.exportToPDF}>Export as PDF</button>
            <button className="export-excel" onClick={this.exportToExcel}>Export as Excel</button>
          </div>
